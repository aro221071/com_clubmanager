 ...
 */
class ClubmanagerViewEmailnewsletters extends JViewLegacy {
    ...
    public function display($tpl = null) {
        ...
        ClubmanagerHelper::addSubmenu('emailnewsletters');
    ...
    protected function addToolbar() {
        ...
        JToolBarHelper::title(JText::_('COM_CLUBMANAGER_TITLE_EMAILNEWSLETTERS'), 'emailnewsletters.png');

        //Check if the form exists before showing the add/edit buttons
        $formPath = JPATH_COMPONENT_ADMINISTRATOR . '/views/emailnewsletter';
        if (file_exists($formPath)) {

            if ($canDo->get('core.create')) {
                JToolBarHelper::addNew('emailnewsletter.add', 'JTOOLBAR_NEW');
            }

            if ($canDo->get('core.edit') && isset($this->items[0])) {
                JToolBarHelper::editList('emailnewsletter.edit', 'JTOOLBAR_EDIT');
            }
        }
        ...
        if ($canDo->get('core.edit.state')) {
            ...
            if (isset($this->items[0]->state)) {
                ...
                JToolBarHelper::custom('emailnewsletters.publish', 'publish.png', 'publish_f2.png', 'JTOOLBAR_PUBLISH', true);
                JToolBarHelper::custom('emailnewsletters.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
            } else if (isset($this->items[0])) {
                //If this component does not use state then show a direct delete button as we can not trash
                JToolBarHelper::deleteList('', 'emailnewsletters.delete', 'JTOOLBAR_DELETE');
            }

            if (isset($this->items[0]->state)) {
                JToolBarHelper::divider();
                JToolBarHelper::archiveList('emailnewsletters.archive', 'JTOOLBAR_ARCHIVE');
            }
            if (isset($this->items[0]->checked_out)) {
                JToolBarHelper::custom('emailnewsletters.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
            }
        }
        ...
        if (isset($this->items[0]->state)) {
            if ($state->get('filter.state') == -2 && $canDo->get('core.delete')) {
                JToolBarHelper::deleteList('', 'emailnewsletters.delete', 'JTOOLBAR_EMPTY_TRASH');
                JToolBarHelper::divider();
            } else if ($canDo->get('core.edit.state')) {
                JToolBarHelper::trash('emailnewsletters.trash', 'JTOOLBAR_TRASH');
                JToolBarHelper::divider();
            }
        }
        ...
        JHtmlSidebar::setAction('index.php?option=com_clubmanager&view=emailnewsletters');
